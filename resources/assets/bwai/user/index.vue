<template>
    <layout>
        ...
        <template #breadcrumbs>
            <bw-breadcrumb>Users</bw-breadcrumb>
        </template>
        ...
        <div class="flex flex-col rounded-lg bg-light dark:bg-dark">
            ...
            <bw-link v-for="user in filteredUsers" :key="user.id" :href="route('dashboard.user.show', user.id)" class="flex flex-col sm:flex-row border-b border-light-100 dark:border-dark-100 hover:bg-white dark:hover:bg-black transition cursor-pointer">
                ...
                <div class="flex flex-1 justify-center items-center p-2 sm:p-4">{{ createdAt(user.created_at) }}</div>
            </bw-link>
...
export default {
  components: { FilterIcon },
    ...
    methods: {
        createdAt(created) {
            return window.moment(created).locale('en').format('LL')
        }
    },
    created() {
        //
